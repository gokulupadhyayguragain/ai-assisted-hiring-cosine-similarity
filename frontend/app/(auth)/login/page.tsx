...
const GOOGLE_LOGIN_URL = `${API_BASE}/api/v1/auth/google/login?mode=login&role=candidate`;
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        router.push(`/role-selection?next=${encodeURIComponent(dashboardPath(user.role))}`);
      } catch (err: any) {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="rounded-3xl border border-gray-200 bg-white p-8 shadow-card">
          <div className="text-center mb-8">
            ...
            <p className="mt-1 text-sm text-gray-500">Sign in to your AIHire account</p>
          </div>
          ...
          <GoogleButton href={GOOGLE_LOGIN_URL} label="Continue with Google" />
