...
export default function HrScreeningPage() {
  ...
  async function run(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.href = "/hr/rankings";
  ...
  return (
    <div className="space-y-6">
      <div className="glass-card p-6">
        <h1 className="display-title text-2xl md:text-3xl text-white">Run AI Screening</h1>
      ...
      <form className="glass-card p-6" onSubmit={run}>
        <label className="space-y-2">
          ...
          <textarea className="field min-h-44" value={jobText} onChange={(e) => setJobText(e.target.value)} placeholder="Paste job description or enter a job ID..." />
        </label>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="rounded-2xl border border-dashed border-white/30 bg-white/5 p-4 text-sm text-zinc-200 cursor-pointer">
            Job File: {jobFile ? jobFile.name : "Attach .pdf / .docx / .txt"}
