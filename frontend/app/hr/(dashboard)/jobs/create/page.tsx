    ...
    try {
      const response = await fetch(editJobId ? `${API_BASE}/api/jobs/${editJobId}` : `${API_BASE}/api/jobs`, {
        ...
        body: JSON.stringify({
          ...
          description: draft.description, created_by: "hr",
        }),
      });
      ...
      <div className="glass-card p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">Define role requirements and save to the database.</p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {[
            ...
            <label key={key} className="space-y-2">
              ...
              <input className="field" value={(draft as any)[key]} onChange={(e) => setDraft({ ...draft, [key]: e.target.value })} />
            </label>
          ))}
          ...
          <label className="space-y-2 md:col-span-2">
            ...
            <input className="field" value={draft.skills} onChange={(e) => setDraft({ ...draft, skills: e.target.value })} placeholder="Python, FastAPI, PostgreSQL" />
          </label>
          <label className="space-y-2 md:col-span-2">
            <span className="text-sm text-zinc-300">Description / JD</span>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <Link href="/hr/screening" className="ghost-btn">Run Screening</Link>
        </div>
      </div>
    </div>

    <Modal open={Boolean(savedJob)} title="Job Saved" onClose={() => setSavedJob(null)}>
      {savedJob && (
        <div className="space-y-3 text-zinc-200">
          <p className="inline-flex items-center gap-2 text-emerald-200"><CheckCircle2 size={16} /> Job saved.</p>
          <p className="text-sm">{savedJob.title}</p>
          <div className="flex gap-2">
            <Link href="/hr/screening" className="primary-btn">Run Screening</Link>
            <Link href="/hr/jobs" className="ghost-btn">Back to Jobs</Link>
          </div>
        </div>
      )}
    </Modal>
