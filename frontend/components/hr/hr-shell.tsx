...
import {
  LayoutDashboard, Briefcase, FileText, BarChart3, ShieldCheck,
  Upload, Settings, Menu, X, LogOut, Brain, Users, GitCompare,
} from "lucide-react";
...
const NAV = [
  ...
  { href: "/hr/compare", label: "Compare Resumes", icon: GitCompare },
    ...
    <div className="min-h-screen bg-ink-950">
      ...
      >
        <div className="flex h-16 items-center gap-2 border-b border-white/10 px-5">
          ...
          <span className="font-display text-lg font-bold text-white">AIHire HR</span>
        </div>
        <nav className="flex h-[calc(100vh-4rem)] flex-col overflow-y-auto p-3">
          <div className="flex-1 space-y-1">
            {NAV.map((item) => {
              const active = pathname === item.href || (item.href !== "/hr" && pathname.startsWith(item.href));
              return (
                <Link
                  key={item.href}
                  href={item.href}
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/10 bg-ink-950/80 px-5 backdrop-blur-xl">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-sm text-zinc-400">HR Workspace</span>
